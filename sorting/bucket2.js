const bucketSort = (givenArray) => {
   ...
   const maxElement = Math.max(...givenArray); // 41
   const minElement = Math.min(...givenArray); // 19
   const base = 10;
  
   const numberOfBucket = Math.floor(maxElement/base) - Math.floor(minElement/base) + 1; // 4
